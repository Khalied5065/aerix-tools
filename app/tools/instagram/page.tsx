...
export default function InstagramSniperPage() {
  ...
  const startScan = async () => {
    ...
    while (checkedCount < count) {
      try {
        const res = await fetch("/api/instagram-sniper", {
          method: "POST",
    ...
    <main className="min-h-screen bg-[#050505] px-6 py-20 text-white">
      <div className="mx-auto flex max-w-6xl flex-col gap-8 lg:flex-row">
        ...
        <section className="w-full rounded-[32px] border border-white/10 bg-white/5 p-8 backdrop-blur-xl lg:w-[45%]">
          ...
          <h1 className="text-3xl font-bold">أداة فحص إنستغرام</h1>
          ...
          <div className="mt-8 space-y-6">
            ...
            <button
              onClick={startScan}
              disabled={isScanning}
              className="w-full rounded-2xl bg-gradient-to-r from-pink-600 to-purple-600 px-5 py-3 font-semibold text-white transition disabled:opacity-50"
            >
